        ...
        def host_thread():
            try:
                ...
                if session_id:
                    ...
                    self.root.after(0, lambda: self.code_display.config(fg=self.colors['accent_green']))
                    self.root.after(0, lambda: self.copy_code_btn.config(state="normal", bg=self.colors['button_bg'], fg='white'))
                    self.root.after(0, lambda: self.host_stop_btn.config(state="normal", bg=self.colors['accent_red'], fg='white'))
                    ...
                    self.root.after(0, lambda: self.log_activity("Failed to create session", "error"))
                    self.root.after(0, lambda: self.reset_host_ui())
                    
            except Exception as e:
                self.root.after(0, lambda: self.log_activity(f"Error: {e}", "error"))
                self.root.after(0, lambda: self.reset_host_ui())
